...
def capture_maskeddense_outputs(masked_layers):
    """
    Context manager that monkeypatches MaskedDense.call for the provided layer instances,
    recording their outputs keyed by unique instance id (layer.name + hex(id(layer))).

    Important:
    - This assumes caller enables eager mode before entering the context so `.numpy()` works.
    - The wrapper calls the original *bound* method unchanged and returns its exact result.
    - Debug assertions added for TransformerBlock / downstream verification.
    """
    records = {}
    originals = {}
    ...
    for layer in masked_layers:
        ...
        records[key] = []
        originals[key] = layer.call

        def make_wrapper(orig_method, layer, key):
            def wrapper(inputs, *args, **kwargs):
                res = orig_method(inputs, *args, **kwargs)
                
                # debug: ensure tuple expected by TransformerBlock
                try:
                    if isinstance(res, (tuple, list)) and len(res) == 2:
                        pre, out = res
                        assert isinstance(pre, tf.Tensor) and isinstance(out, tf.Tensor)
                        out_tensor = out
                    else:
                        out_tensor = res
                except Exception:
