"""adaptive_overparam_experiment_v34.py"""
...
class NeuroAdaptiveCREC:
    ...
    def calibrate_targets(self, fisher_metric, mask_entropies):
        ...
        self.fisher_history.append(fisher_metric)
        self.entropy_history.append(np.mean(mask_entropies))
        ...
        self.fisher_target = np.median(self.fisher_history) * self.fisher_target_scale
        # Dynamic entropy target = smooth mix between current mean and theoretical max (ln(2))
        mean_ent = np.mean(self.entropy_history)
        self.entropy_target_dynamic = 0.8 * mean_ent + 0.2 * np.log(2)
    ...
    def update(self, fisher_metric, val_delta, masks):
        ...
        mask_entropies = [self._mask_entropy(m) for m in masks if m.size > 0]
        print(f'\nmask_entropies:\n{mask_entropies}\n')
        mean_entropy = np.mean(mask_entropies)
        ...
        # If Fisher too high -> increase compression
        # If entropy too low -> decrease compression to allow exploration
        compression_adjust = (
...
class EvoCompressor:
    ...
    def evaluate_and_compress(self, train_loss: Optional[float] = None, val_loss: Optional[float] = None, do_regrow: bool = True) -> Tuple[Dict[str, Any], float]:
        ...
        print(f'\mask_entropies: {mask_entropies}\n')
