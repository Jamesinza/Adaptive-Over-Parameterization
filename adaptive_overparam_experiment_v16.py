"""
adaptive_overparam_experiment_v16.py
...
def capture_maskeddense_outputs(masked_layers):
    ...
    for layer in masked_layers:
        ...
        def make_wrapper(orig_method, layer_ref, key_ref):
...
def approx_fisher_per_sample(model, dataset, loss_fn, max_samples_per_batch=8, batch_size=None):
    """
    Robust per-sample approximate Fisher accumulation.
    - Builds (when possible) functional submodels to retrieve intermediate outputs.
    - For compatibility with subclassed layers, falls back to calling the full model and submodel extraction.
    - Temporarily enables eager execution to avoid autograph pitfalls when computing per-example gradients.
    NOTE: still relatively slow for large datasets; keep max_samples_per_batch small (<=8) for prototyping.
    """
    ...
    if max_samples_per_batch > 8:
        print("approx_fisher_per_sample: Warning — large max_samples_per_batch will be slow. Recommend <=8.")
    ...
    for x_batch, y_batch in batch_iter:
        ...
        # ensure eager when doing per-example tape operations for robustness
        prev = bool(getattr(tf.config, "functions_run_eagerly", lambda: False)())
        tf.config.run_functions_eagerly(True)
        try:
            def _per_example_grad(x_i, y_i):
                x_i = tf.expand_dims(x_i, 0)
                y_i = tf.expand_dims(y_i, 0)
                with tf.GradientTape() as tape:
                    preds = model(x_i, training=False)
                    outs = {}
                    # extract intermediate outputs when submodel present
                    for l in md_layers:
                        key = f"{l.name}_{hex(id(l))}"
                ...
                for key, out in outs.items():
                    g = tape.gradient(loss, out)
                    grads_map[key] = g
                return grads_map

            for i in range(n_samples):
                grads_map = _per_example_grad(x_small[i], y_small[i])
                for key, g in grads_map.items():
                    if g is None:
                        continue
                    g_np = g.numpy()
                    # collapse gradients to unit-level contributions
                    if g_np.ndim == 3:
                        unit_grad = np.sqrt(np.sum(g_np[0] ** 2, axis=0))
                    elif g_np.ndim == 2:
                        unit_grad = np.abs(g_np[0])
                    else:
                        unit_grad = np.abs(g_np.ravel())
