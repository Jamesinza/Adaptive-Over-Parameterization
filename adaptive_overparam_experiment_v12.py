...
CONFIG = {
    ...
    'prune_fraction': 0.3,
    'regrow_fraction': 0.3,
    'connection_rewire_fraction': 0.05,
    'd_model': 32,
    'hidden_units': 128,
    'hidden_layers': 2,
    'num_heads': 2,
...
class MaskedDense(keras.layers.Layer):
    ...
    # NEW: connection-level operations
...
class EvoCompressor:
    ...
    def evaluate_and_compress(self, train_loss=None, val_loss=None):
        ...
        for unique_name, score in utilities.items():
            ...
            # ---- connection-level rewiring ----
            conn_util = np.abs(layer.kernel.numpy()) * conn_mask  # simple proxy
            flat_util = conn_util.flatten()
            n_rewire = int(flat_util.size * self.connection_rewire_fraction)
            if n_rewire > 0:
                conn_alive_before = int(conn_mask.sum())
                avg_mag_before = float(np.mean(np.abs(layer.kernel.numpy()[conn_mask > 0.5])))
            
                low_idx = np.argpartition(flat_util, n_rewire)[:n_rewire]
                low_pairs = np.array(np.unravel_index(low_idx, conn_mask.shape)).T
            
                # prune selected weak connections
                layer.prune_connections(low_pairs)
            
                # choose same number of currently dead connections to regrow
                dead_pairs = np.argwhere(conn_mask < 0.5)
                if len(dead_pairs) > 0:
                    regrow_idx = np.random.choice(len(dead_pairs), size=min(n_rewire, len(dead_pairs)), replace=False)
                    regrow_pairs = dead_pairs[regrow_idx]
                    layer.regrow_connections(regrow_pairs)
            
                conn_alive_after = int(layer.conn_mask.numpy().sum())
                avg_mag_after = float(np.mean(np.abs(layer.kernel.numpy()[layer.conn_mask.numpy() > 0.5])))
            
                print(f"\n[DEBUG] {layer.name}: connections rewired={n_rewire} | "
                      f"alive before={conn_alive_before}, after={conn_alive_after} | "
                      f"avg|w| before={avg_mag_before:.4f}, after={avg_mag_after:.4f}")
            else:
                print(f"\n[DEBUG] {layer.name}: skipping connection rewiring (n_rewire={n_rewire})")
        ...
        for name, layer in [(n, next((l for l in md_layers if f"{l.name}_{hex(id(l))}" == n), None)) for n in utilities.keys()]:
            ...
            conn_alive = int(layer.conn_mask.numpy().sum())
            conn_density = conn_alive / np.prod(layer.conn_mask.shape)
            avg_conn_mag = float(np.mean(np.abs(layer.kernel.numpy()[layer.conn_mask.numpy() > 0.5])))
            ...
            df_rows.append({
                ...
                'utility_mean': float(utilities[name].mean()),
