"""
adaptive_overparam_experiment_v11.py
...
class EvoCompressor:
    def __init__(self, model, val_ds, base_prune=0.2, regrow_fraction=0.2,
                 connection_rewire_fraction=0.03, log_dir=None):
        self.model = model
        self.val_ds = val_ds
    ...
    def evaluate_and_compress(self, train_loss=None, val_loss=None):
        ...
        # --- dynamic prune fraction based on gen gap ---
        ...
        # === per-layer evolutionary compression ===
        total_alive = 0
        for unique_name, score in utilities.items():
            ...
            print(f'\nLayer {unique_name}: neuron mask (alive count)= {int(mask.sum())}\tconnection mask (alive count)= {int(conn_mask.sum())}')
            ...
            # stochastic regrow: number of regrows ~ Binomial(n_prune, regrow_fraction)
            if self.regrow_fraction > 0:
                # this yields 0..n_prune and scales sensibly even for n_prune == 1
                n_regrow = np.random.binomial(n_prune, self.regrow_fraction)
            else:
                n_regrow = 0
            
            if n_regrow > 0:
                # choose regrow indices from the dead pool instead of only the just-pruned ones:
                ...
                print(f"[DEBUG] {layer.name}: pruned {len(prune_idx)} -> regrew {len(regrow_idx)} (stochastic)")

            # ---- connection-level rewiring: entropy+Fisher guided regrowth ----
            W = layer.kernel.numpy()
            conn_mask = layer.conn_mask.numpy()
            absW = np.abs(W) * (conn_mask > 0.5)  # only alive magnitudes for ranking prunes
            flat_absW = absW.flatten()
            
            n_rewire = int(flat_absW.size * self.connection_rewire_fraction)
            
            if n_rewire > 0:
                conn_alive_before = int(conn_mask.sum())
                # 1) prune weakest connections by absolute magnitude (fast proxy)
                low_idx = np.argpartition(flat_absW, n_rewire)[:n_rewire]
                low_pairs = np.array(np.unravel_index(low_idx, conn_mask.shape)).T
                layer.prune_connections(low_pairs)
            
                # 2) build per-output (column) utility from precomputed utilities dict
                # utilities[unique_name] is a 1D array length = layer.units
                # normalize to [0,1]
                ...
                # 3) compute per-input proxy utility: mean absolute incoming weight (normalized)
                # This is a cheap proxy for input-side activity / usefulness.
                in_u = np.mean(np.abs(W), axis=1)  # shape (in_dim,)
                in_u = (in_u - in_u.min()) / (in_u.max() - in_u.min() + 1e-9)
            
                # 4) construct connection-level desirability score for dead connections
                #    U_ij = |w_ij| * (alpha_out * out_u_j + beta_in * in_u_i)
                #    since dead connections have w ~ 0, we use the post-prune absW for existing weights,
                #    but for dead positions use small epsilon so product isn't identically zero.
                alpha_out = 0.6
                beta_in = 0.4
                eps = 1e-6
            
                # find dead connection positions
                dead_pairs = np.argwhere(conn_mask < 0.5)
                if dead_pairs.size == 0:
                    regrew = 0
                else:
                    # compute desirability for each dead pair
                    # vectorized:
                    di = dead_pairs[:, 0]
                    dj = dead_pairs[:, 1]
                    desirability = (alpha_out * out_u[dj]) + (beta_in * in_u[di])
                    # break ties using potential magnitude of weight (we'll simulate with small random noise)
                    desirability += np.random.rand(len(desirability)) * 1e-6
            
                    # select top-k desirable dead pairs to regrow (k = n_rewire or available dead count)
                    k = min(n_rewire, len(dead_pairs))
                    choose_idx = np.argpartition(-desirability, k - 1)[:k]
                    regrow_pairs = dead_pairs[choose_idx]
            
                    # 5) perform regrowth and weight init
                    layer.regrow_connections(regrow_pairs)
                    regrew = len(regrow_pairs)
            
                conn_alive_after = int(layer.conn_mask.numpy().sum())
                # compute average magnitude on alive connections (safe guard for empty)
                alive_mask = layer.conn_mask.numpy() > 0.5
                if alive_mask.sum() > 0:
                    avg_mag_before = float(np.mean(np.abs(W[alive_mask])))
                    avg_mag_after = float(np.mean(np.abs(layer.kernel.numpy()[alive_mask])))
                else:
                    avg_mag_before = avg_mag_after = 0.0
            
                print(f"[GUIDED_REWIRE] {layer.name}: rewired={n_rewire} | pruned_low={len(low_pairs)} "
                      f"| regrew={regrew} | conn_alive {conn_alive_before}->{conn_alive_after} "
                      f"| avg|w| before={avg_mag_before:.6f}, after={avg_mag_after:.6f}")
            else:
                print(f"[GUIDED_REWIRE] {layer.name}: skipping (n_rewire={n_rewire})")
        ...
        summary = {'alive_total': int(total_alive),
        ...
        for name, layer in [(n, next((l for l in md_layers if f"{l.name}_{hex(id(l))}" == n), None)) for n in utilities.keys()]:
            ...
            alive_neurons = int(layer.mask.numpy().sum())
            ...
            df_rows.append({
                ...
                'utility_mean': float(utilities[name].mean()) if name in utilities else 0.0,
