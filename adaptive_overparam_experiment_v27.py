...
class EvoConfig:
    ...
    compress_policy: str = 'rollback'            # 'rollback' | 'accept_if_compression_win' | 'try_longer_retrain'
    ...
    ablation_max_batches: int = 4                # val batches to use for ablation scoring
    ablation_max_neurons: int = 32               # max neurons to test per layer
    ablation_weight: float = 0.5                 # how much ablation contributes to unit score
    ema_decay: float = 0.5                       # EMA smoothing factor for unit utilities
    deterministic_regrow_seed: int = SEED        # base seed for deterministic regrow reinit
    regrow_reinit_std: float = 0.05              # relative std when reinitializing regrown connections
