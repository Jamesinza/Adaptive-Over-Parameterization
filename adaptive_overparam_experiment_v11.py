...
class EvoCompressor:
    ...
    def evaluate_and_compress(self, train_loss=None, val_loss=None):
        ...
        for unique_name, score in utilities.items():
            ...
            # Optional safety: ensure some minimal exploration over time
            # Example: force at least one regrow every K cycles — requires compressor to track cycles
            # if n_regrow == 0 and (self.history and len(self.history) % 5 == 0):
            #     n_regrow = 1
            ...
            if n_regrow > 0:
                ...
                # # For simplicity, regrow from prune_idx:
                # regrow_idx = np.random.choice(prune_idx, size=min(n_regrow, len(prune_idx)), replace=False)
