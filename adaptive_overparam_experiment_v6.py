"""
adaptive_overparam_experiment_v6.py
...
class TransformerBlock(keras.layers.Layer):
    def __init__(self, d_model, num_heads, ff_dim, block_id, name=None):
        ...
        self.ffn_dense1 = MaskedDense(ff_dim, activation='gelu', name=f'ffn_dense1_{self.block_id}')
        self.ffn_dense2 = MaskedDense(d_model, activation=None, name=f'ffn_dense2_{self.block_id}')
...
def capture_activations(model, dataset, masked_layers=None):
    ...
    for l in masked_layers:
        try:
            out_tensor = l.output
            print(f"\nCollecting activations for {l.name}, shape={out_tensor.shape}\n")
    ...
    need_monkey = [l for l in masked_layers if submodels.get(l.name) is None]
    ...
    for x, y in dataset:
        ...
        if need_monkey:
            with capture_maskeddense_outputs(need_monkey) as records:
                ...
                for name, outs in records.items():
                    if not outs:
                        continue
                    arrs = []
                    for o in outs:
                        ...
                        arrs.append(o_np)
                    if arrs:
                        acts[name].append(np.concatenate(arrs, axis=0))

    final = {}
    for l in masked_layers:
        name = l.name
        if len(acts[name]) == 0:
            final[name] = np.zeros((0, l.units), dtype=np.float32)
        else:
            final[name] = np.concatenate(acts[name], axis=0)
    return final
