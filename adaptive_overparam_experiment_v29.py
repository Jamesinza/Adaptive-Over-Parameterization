"""adaptive_overparam_experiment_v29.py"""
...
CONFIG = {
    ...
    'num_heads': 8,
    'learning_rate': 1e-3,
    'max_fisher_samples_per_batch': 32,
    'max_grad_samples_per_batch': 32,
    'cycle_patience': 30,
...
class EvoConfig:
    ...
    grad_unit_weight: float = 0.5
    grad_weight_weight: float = 0.3
    fisher_weight: float = 0.5
    max_grad_samples: int = 32                   # cap number of val batches to use for gradient-based scoring
    ...
    ablation_weight: float = 0.5                 # how much ablation contributes to unit score
...
class EvoCompressor:
    ...
    def compute_utilities(self, md_layers) -> Dict[str, np.ndarray]:
        ...
        Fixed: compute per-sample activation and weight gradients inside the same forward/tape
        (uses a small per-batch prefix and tf.vectorized_map to obtain per-sample grads).
        """
        ...
        fisher = approx_fisher_per_sample(
            ...
            max_samples_per_batch=CONFIG.get('max_fisher_samples_per_batch', 8)
        )
        ...
        # ------------------ build per-layer submodels (with warmup materialization) --------------
        layer_submodels = {}
        failed_layers = []
        ...
        # If any failed because layer.output doesn't exist, run a single forward pass to materialize.
        if failed_layers:
            self.log.debug(f'{len(failed_layers)} layers uninitialized. Attempting a warm-up forward pass to materialize layer outputs.')
        
            # helper to get one sample from val_ds (numpy tuple or tf.data)
            def _get_sample(val_ds):
                try:
                    if isinstance(val_ds, tuple) and isinstance(val_ds[0], np.ndarray):
                        X_val, y_val = val_ds
                        if X_val.shape[0] == 0:
                            raise ValueError("Empty X_val")
                        return X_val[:1], y_val[:1]
                    else:
                        it = iter(val_ds)
                        xb, yb = next(it)
                        # ensure numpy arrays or tensors; take first sample
                        xb = np.asarray(xb)
                        yb = np.asarray(yb)
                        return xb[:1], yb[:1]
                except Exception:
                    # fallback: synthetic zeros using CONFIG shapes (safe fallback)
                    seq_len = int(CONFIG.get('seq_len', 1))
                    feat = int(CONFIG.get('feature_dim', 1)) if CONFIG.get('feature_dim') else 1
                    # try to infer input shape from model input(s)
                    try:
                        inp_shape = self.model.input_shape
                        # pick a sensible shape if available
                        if isinstance(inp_shape, tuple):
                            shape = tuple([1 if d is None else d for d in inp_shape])
                        elif isinstance(inp_shape, list) and len(inp_shape) > 0:
                            shape = tuple([1 if d is None else d for d in inp_shape[0]])
                        else:
                            shape = (1, seq_len, feat)
                    except Exception:
                        shape = (1, seq_len, feat)
                    return np.zeros(shape, dtype=np.float32), np.zeros((1,), dtype=np.float32)
        
            x_sample, y_sample = _get_sample(self.val_ds)
        
            # run a warm-up forward pass; force eager during warmup to avoid tracing issues for subclassed models
            prev_eager = bool(getattr(tf.config, "functions_run_eagerly", lambda: False)())
            tf.config.run_functions_eagerly(True)
            try:
                try:
                    _ = self.model(tf.convert_to_tensor(x_sample, dtype=tf.float32), training=False)
                    self.log.debug('Warm-up forward pass successful.')
                except Exception as e:
                    self.log.debug(f'Warm-up forward pass failed: {e}')
            finally:
                # restore eager setting
                tf.config.run_functions_eagerly(prev_eager)
        
            # retry only failed submodels
            remaining = []
            for layer in failed_layers:
                unique_name = f"{layer.name}_{hex(id(layer))}"
                try:
                    layer_submodels[unique_name] = keras.Model(self.model.inputs, [layer.output, self.model.output])
                    self.log.debug(f'Rebuilt submodel for {unique_name} after warmup.')
                except Exception as e:
                    self.log.debug(f'Still failed to build submodel for {unique_name} after warmup: {e}')
                    layer_submodels[unique_name] = None
                    remaining.append(layer)
        
            if remaining:
                self.log.debug(f'{len(remaining)} layers remain un-buildable; falling back to capture/jacobian path for those layers.')
        
    
        # # Build per-layer functional submodels where possible (expose layer.activation and full output)
        # layer_submodels = {}
        # per_layer_failed = set()
        # for layer in md_layers:
        #     unique_name = f"{layer.name}_{hex(id(layer))}"
        #     try:
        #         layer_submodels[unique_name] = keras.Model(self.model.inputs, [layer.output, self.model.output])
        #     except Exception as e:
        #         # don't disable grad for *all* layers, just mark this one as failed
        #         self.log.debug(f'Failed to build submodel for {unique_name}: {e}')
        #         layer_submodels[unique_name] = None
        #         per_layer_failed.add(unique_name)
    
        batches_done = 0
        loss_fn = keras.losses.get(keras.losses.Huber())
    
        # small per-sample prefix used for accurate per-sample grads (limit memory)
        max_samples_per_batch = int(CONFIG.get('max_grad_samples_per_batch', 8))
        ...
        for batch in val_iter:
            ...
            # take a small prefix for per-sample operations
            n_samples = int(min(int(B), max_samples_per_batch))
            x_small = xb_tf[:n_samples]
            y_small = yb_tf[:n_samples]
    
            if use_grad and layer_submodels:
                # For each layer, compute per-sample unit importance (abs(act * dL/dact))
                # and per-sample kernel grads (if layer has kernel), all inside single tape per-example.
                for layer in md_layers:
                    unique_name = f"{layer.name}_{hex(id(layer))}"
                    subm = layer_submodels.get(unique_name)
                    if subm is None:
                        print(f"\nfallback: can't compute per-layer grads for {unique_name} layer\n")
                        continue
    
                    # If layer has kernel, we'll compute kernel grads per-sample; else skip weight grad.
                    has_kernel = hasattr(layer, 'kernel') and layer.kernel is not None
    
                    # Define per-example function (no explicit tf.function wrapper to avoid complex signatures;
                    # we rely on vectorized_map + small n_samples to control cost).
                    def per_example_fn(xi, yi, _subm=subm, _layer=layer):
                        xi = tf.expand_dims(xi, 0)   # (1, seq_len, feat...) or (1, features)
                        yi = tf.expand_dims(yi, 0)
                        with tf.GradientTape() as tape:
                            act_b, pred_b = _subm(xi, training=False)  # act_b: (1,...,units)
                            tape.watch(act_b)
                            # loss_b is scalar for this single example
                            loss_b = tf.reshape(loss_fn(yi, pred_b), [])
                        # grad wrt activation (per-sample)
                        g_act = tape.gradient(loss_b, act_b)
                        # collapse to per-unit magnitude like your earlier logic
                        if g_act is None:
                            # return zeros of correct units
                            # attempt to infer units from act_b
                            units = tf.shape(act_b)[-1]
                            unit_mag = tf.zeros((units,), dtype=tf.float32)
                        else:
                            g0 = g_act[0]  # remove batch dim -> (..., units) or (units,)
                            a0 = act_b[0]
                            # compute elementwise act * grad and collapse temporal dims (if any)
                            prod = tf.abs(a0 * g0)
                            if tf.rank(prod) == 1:
                                unit_mag = prod
                            else:
                                # collapse all but last (units) dims
                                reduce_axes = tf.range(tf.rank(prod) - 1)
                                unit_mag = tf.sqrt(tf.reduce_sum(tf.square(prod), axis=reduce_axes))
                        # kernel grad per-sample if available
                        if has_kernel:
                            g_k = tape.gradient(loss_b, _layer.kernel)  # gradient shape == kernel.shape
                            if g_k is None:
                                # return zeros same shape as kernel
                                g_k = tf.zeros_like(_layer.kernel)
                        else:
                            g_k = tf.constant([], dtype=tf.float32)  # empty tensor as placeholder
                        return unit_mag, g_k
    
                    # run vectorized_map; this produces (n_samples, units), (n_samples, *kernel_shape) or empty
                    try:
                        mapped = tf.vectorized_map(lambda elems: per_example_fn(elems[0], elems[1]), (x_small, y_small))
                        per_sample_unit = mapped[0]  # shape (n_samples, units)
                        per_sample_k = mapped[1]     # shape (n_samples, *kernel_shape) or empty
                    except Exception as e:
                        # fallback to python loop if vectorized_map fails
                        per_unit_list = []
                        per_k_list = []
                    ...
                    # aggregate per-unit importance: mean over per-sample prefix
                    per_unit_avg = tf.reduce_mean(per_sample_unit, axis=0).numpy()  # (units,)
                    ...
                    # aggregate per-connection importance if kernel exists
                    if has_kernel:
                        # per_sample_k shape (n_samples, ...) -> average across n_samples
                        avg_grad_k = tf.reduce_mean(per_sample_k, axis=0).numpy()  # shape == kernel.shape
                        try:
                            kernel_np = layer.kernel.numpy()
                            w_imp = np.abs(kernel_np * avg_grad_k)  # per-connection importance
                        except Exception:
                            # in case kernel numpy() fails
        ...
        # finalize accumulators (average over batches seen)
        for layer in md_layers:
            unique_name = f"{layer.name}_{hex(id(layer))}"
            ...
            if grad_weight_accum.get(unique_name) is not None:
                ...
                # attempt to create zero array same shape as kernel if available
                try:
            ...
            # debug dumps
            self.log.debug(f'Layer {unique_name} — gunit mean {np.mean(gunit) if gunit.size else "NA"}; gweight mean {np.mean(gweight) if gweight.size else "NA"}')
            ...
            # combine per-unit importance (grad_unit + fisher) first pass
            alpha = float(self.config.grad_unit_weight)
            beta = float(self.config.fisher_weight)
            gamma = float(self.config.ablation_weight)  # ablation will be added below
        ...
        # compute ablation scores (cheap) and incorporate
        try:
        ...
        # combine final signals (with EMA)
        for layer in md_layers:
            unique_name = f"{layer.name}_{hex(id(layer))}"
    
            # retrieve raw components (raw gunit, fisher, ablation)
            ...
            alpha = float(self.config.grad_unit_weight)
            beta = float(self.config.fisher_weight)
            gamma = float(self.config.ablation_weight)
...
def approx_fisher_per_sample(model, dataset, loss_fn, max_samples_per_batch=8, batch_size=None):
    ...
    for l in md_layers:
        ...
        try:
            submodels[key] = keras.Model(model.input, l.output)
            # print('\nsubmodels[key] correctly captured for fisher run...\n')
        except Exception:
            # print('\nNo submodels[key] captured for fisher run...\n')
            submodels[key] = None
    ...
    for x_batch, y_batch in batch_iter:
        ...
        if need_jacobian:
            # print('\nTriggered jacobian in fisher...\n')
            # Ensure eager so capture_maskeddense_outputs records actual tensors
            prev = bool(getattr(tf.config, "functions_run_eagerly", lambda: False)())
            tf.config.run_functions_eagerly(True)
            try:
                # Run model once inside both the capture context and the tape so captured outs
                # are connected to preds in the recorded graph.
                with capture_maskeddense_outputs([l for (l, _) in need_jacobian], record_tensors=True) as records:
                    with tf.GradientTape(persistent=True) as tape:
                        preds = model(x_small, training=False)                # forward pass recorded by tape
                        # grab the captured tensors that correspond to this forward pass
                        outs = {}
                        for l, key in need_jacobian:
                            rec = records.get(key, [])
                            if not rec:
                                continue
                            outs[key] = rec[0]   # tensor from same forward pass
                        # watch the captured outputs so tape can compute jacobians wrt them
            ...
            if outs:
                for key, out_t in outs.items():
                    # compute jacobian: gives shape typically (n_samples, n_samples, ...out_t.shape[1:])
                    ...
                    # We want per-sample grads: pick diagonal elements jac[i, i, ...]
                    # Handle dynamic ranks robustly.
                    try:
                        # assume jac shape = (n_samples, n_samples, *out_tail)
                        n0 = tf.shape(jac)[0]
                        # stack diagonal entries: resulting shape (n_samples, *out_tail)
                        diag_list = [jac[i, i] for i in range(int(n0))]
                        per_sample = tf.stack(diag_list, axis=0)
                    except Exception:
                        # fallback: gather_nd approach (works when first two dims are n_samples)
                        idx = tf.stack([tf.range(n_samples), tf.range(n_samples)], axis=1)  # (n,2)
                        # reshape jac to (n_samples, n_samples, -1) then gather diag and restore tail dims if needed
                        flat_tail = tf.reshape(jac, (n_samples, n_samples, -1))
                        per_sample_flat = tf.gather_nd(flat_tail, idx)  # (n_samples, P_flat)
                        # try to recover last dimension = units if possible
                        tail_prod = tf.shape(out_t)[1] if tf.rank(out_t) > 1 else 1
                        per_sample = tf.reshape(per_sample_flat, tf.concat([[n_samples], [-1, tail_prod]][:2], axis=0))
        
                    # collapse temporal dims (all axes except last) into per-unit magnitude
                    if tf.rank(per_sample) > 2:
                        sq = tf.square(per_sample)
                        # sum over axes 1 .. rank-2
                        reduce_axes = list(range(1, tf.rank(per_sample) - 1))
                        unit_sq = tf.reduce_sum(sq, axis=reduce_axes)  # (n_samples, units)
                    else:
                        unit_sq = tf.square(per_sample)  # (n_samples, units) or (n_samples,)
        
                    batch_sum = tf.reduce_sum(unit_sq, axis=0)  # (units,)
                    try:
                        fisher[key] += batch_sum.numpy()
                    except Exception:
                        fisher[key] += np.array(batch_sum)
