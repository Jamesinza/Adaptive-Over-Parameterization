"""
adaptive_overparam_experiment_v17.py
...
class TransformerBlock(keras.layers.Layer):
    ...
    def call(self, x, training=False):
        ...
        # flatten sequence for MaskedDense
        x_flat = tf.reshape(x, (-1, tf.shape(x)[-1]))
        pre, h = self.ffn_dense1(x_flat, return_pre_activation=True)

        # --- robust broadcasting: expand -> tile -> flatten ---
        batch_size = tf.shape(gates)[0]
        ff_dim = tf.shape(gates)[1]
        gates_exp = tf.reshape(gates, (batch_size, 1, ff_dim))          # (batch, 1, ff_dim)
        gates_tiled = tf.tile(gates_exp, [1, seq_len, 1])               # (batch, seq_len, ff_dim)
        gates_flat = tf.reshape(gates_tiled, (-1, ff_dim))             # (batch*seq_len, ff_dim)
...
def approx_fisher_per_sample(model, dataset, loss_fn, max_samples_per_batch=8, batch_size=None):
    """
    Per-batch, vectorized approx Fisher estimator.

    Strategy:
      - For each batch, use only first `n_samples = min(batch, max_samples_per_batch)`.
      - For layers where a functional submodel can be built, compute per-example gradients via
        tf.vectorized_map over a small per-example function that opens a GradientTape and computes
        grad(loss_scalar, layer_output) for that example. Collapse gradients to unit-level magnitudes.
      - For layers where a submodel cannot be built (subclassed layers), fall back to
        capturing the batch output by monkeypatching and use a single tape + tape.jacobian and extract the diagonal.
    """
    md_layers = get_masked_dense_layers(model)
    if batch_size is None:
        batch_size = CONFIG['batch_size']

    # try to create functional submodels for each masked layer
    submodels = {}
    for l in md_layers:
        key = f"{l.name}_{hex(id(l))}"
        try:
    ...
    # iterator selection (NumPy tuple or tf.data)
    if isinstance(dataset, tuple) and isinstance(dataset[0], np.ndarray):
        batch_iter = numpy_batch_iter(dataset[0], dataset[1], batch_size)
    else:
        batch_iter = iter(dataset)
    ...
    for x_batch, y_batch in batch_iter:
        ...
        # Only use a small prefix of the batch to limit memory footprint
        x_small = x_batch_tf[:n_samples]
        y_small = y_batch_tf[:n_samples]

        # Partition layers into two groups:
        #  A) have functional submodel -> we can use vectorized_map per-example
        #  B) no functional submodel -> need monkeypatch + jacobian fallback
        have_subm = []
        need_jacobian = []
        for l in md_layers:
            key = f"{l.name}_{hex(id(l))}"
            if submodels.get(key) is not None:
                have_subm.append((l, key, submodels[key]))
            else:
                need_jacobian.append((l, key))

        # --------- Group A: vectorized_map per-example grads for layers with submodels ----------
        # We'll compute per-example unit-wise gradients for each layer independently.
        for l, key, subm in have_subm:
            # Determine expected unit dimension (last dim of submodel output)
            try:
                sample_out = subm(x_small[:1], training=False)
            except Exception:
                # Can't safely call submodel here; fall back to jacobian later
                need_jacobian.append((l, key))
                continue

            # units is last dimension of sample_out
            out_shape = sample_out.shape  # e.g. (1, seq_len, units) or (1, units)
            units = int(out_shape[-1])

            # per-example function: compute unit-wise gradient magnitude for this layer output
            def per_example_fn(x_i, y_i):
                # x_i: single sample shape (seq_len, features), y_i: scalar/shape ()
                x_i = tf.expand_dims(x_i, 0)  # make batch dim 1
                y_i = tf.expand_dims(y_i, 0)
                with tf.GradientTape() as tape:
                    out_i = subm(x_i, training=False)   # shape (1, ... , units)
                    tape.watch(out_i)
                    pred_i = model(x_i, training=False)  # scalar output shape (1,1) or (1,)
                    loss_i = tf.reshape(loss_fn(y_i, pred_i), [])  # scalar
                g = tape.gradient(loss_i, out_i)
                if g is None:
                    return tf.zeros((units,), dtype=tf.float32)
                # collapse g to unit-level magnitude (abs or sqrt-sum-squares)
                g_np = g[0]  # remove leading batch dim -> shape (..., units)
                # If g_np has rank>1 (e.g., seq_len x units), collapse other dims except last
                if len(g_np.shape) == 1:
                    unit_mag = tf.abs(g_np)  # (units,)
                else:
                    # compute sqrt(sum(square) over all but last axis)
                    sq = tf.square(g_np)
                    reduce_axes = list(range(len(g_np.shape)-1))  # collapse axes 0..-2
                    unit_mag = tf.sqrt(tf.reduce_sum(sq, axis=reduce_axes))  # (units,)
                return unit_mag  # shape (units,)

            # vectorized_map expects a function that accepts a *single* tensor or nested structure
            def vmapped(elems):
                x_i, y_i = elems
                return per_example_fn(x_i, y_i)

            # run vectorized_map across the small batch
            try:
                per_example_units = tf.vectorized_map(vmapped, (x_small, y_small))  # shape (n_samples, units)
            except Exception:
                # if vectorized_map fails for some reason, fall back to per-sample python loop (slower)
                per_list = []
                for i in range(n_samples):
                    per_list.append(per_example_fn(x_small[i], y_small[i]))
                per_example_units = tf.stack(per_list, axis=0)

            # square and sum across samples to accumulate Fisher (sum of squared gradients)
        ...
        # --------- Group B: monkeypatch + single-tape jacobian fallback ----------
        if len(need_jacobian) > 0:
            # capture tensors for the batch by monkeypatching subclassed MaskedDense.call
            # ensure eager for safe monkeypatch capture
            prev = bool(getattr(tf.config, "functions_run_eagerly", lambda: False)())
            tf.config.run_functions_eagerly(True)
            try:
                with capture_maskeddense_outputs([l for (l, _) in [(x[0], x[1]) for x in need_jacobian]], record_tensors=True) as records:
                    _ = model(x_small, training=False)
                    # now `records` has raw tensors captured per layer (batch)
                    outs = {}
                    for (l, key) in need_jacobian:
                        rec = records.get(key, [])
                        if not rec:
                            continue
                        outs[key] = rec[0]  # tensor for the full small-batch forward

                # If we obtained any tensors, compute jacobian-based per-sample grads
                if outs:
                    with tf.GradientTape() as tape:
                    ...
                    for key, out_t in outs.items():
                        # jac shape: (n_samples, n_samples, ...out_t.shape[1:])
                        try:
                            jac = tape.jacobian(loss_vec, out_t)
                        except Exception:
                            # If jacobian fails (e.g. resource limitations), skip this layer gracefully
                            continue
            
                        # jac may be None (disconnected) — skip in that case
                        if jac is None:
                            continue
            
                        # Collapse trailing dims into a single dimension with -1 to avoid relying on static shape info.
                        # jac has shape (n_samples, n_samples, ...P...), so reshape to (n_samples, n_samples, P_flat)
                        try:
                            jac_flat = tf.reshape(jac, (n_samples, n_samples, -1))  # (n,n,P_flat)
                        except Exception:
                            # As a very robust fallback, convert dynamic shapes at runtime
                            jac_flat = tf.reshape(jac, tf.concat([[n_samples], [n_samples], [-1]], axis=0))
            
                        # gather diagonal elements (i,i,:) -> per-sample flattened grads
                        idx = tf.stack([tf.range(n_samples), tf.range(n_samples)], axis=1)
                        per_sample_flat = tf.gather_nd(jac_flat, idx)  # (n_samples, P_flat)
            
                        # Determine original trailing shape dynamically and reshape back when possible.
                        # Use tf.shape on out_t to get runtime dims (first dim is batch).
                        out_dyn_shape = tf.shape(out_t)  # (n_samples, *rest)
                        rest_shape = out_dyn_shape[1:]   # runtime shape of trailing dims
            
                        # If there are trailing dims, reshape back; otherwise per_sample_flat already correct.
                        if tf.size(rest_shape) > 0:
                            # Build new shape: [n_samples] + list(rest_shape)
                            new_shape = tf.concat([[n_samples], rest_shape], axis=0)
                            # Note: per_sample_flat currently is (n_samples, P_flat). Reshape using dynamic new_shape.
                            per_sample = tf.reshape(per_sample_flat, new_shape)  # (n_samples, ..., units)
                            # Collapse non-last dims (time/seq) into unit-wise contributions:
                            nd = tf.rank(per_sample)
                            if nd == 2:
                                # already (n_samples, units)
                                unit_sq = tf.square(per_sample)
                            else:
                                sq = tf.square(per_sample)
                                # sum over axes 1..(nd-2) to collapse time/seq dims, keep last dim (units)
                                reduce_axes = tf.range(1, nd - 1)
                                unit_sq = tf.reduce_sum(sq, axis=reduce_axes)
                        else:
                            # out_t was scalar per sample; treat per_sample_flat as (n_samples, 1)
                            unit_sq = tf.square(per_sample_flat)
            
                        # unit_sq has shape (n_samples, units)
                        # sum across samples to produce accumulated squared gradient per unit
