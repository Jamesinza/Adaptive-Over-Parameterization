"""adaptive_overparam_experiment_v28.py"""
...
CONFIG = {
    ...
    'num_heads': 8,
    'learning_rate': 1e-3,
    'max_fisher_samples_per_batch': 512,
    'cycle_patience': 30,
...
class EvoCompressor:
    ...
    def compute_utilities(self, md_layers) -> Dict[str, np.ndarray]:
        ...
        for layer in md_layers:
            ...
            # approx_ablation_scores(self.model, layer)
        ...
        for layer in md_layers:
            unique_name = f"{layer.name}_{hex(id(layer))}"
            # gather normalized components (gunit_n, f_n already computed above)
            gunit_n = getattr(layer, '_last_grad_unit_importance', None)
            if gunit_n is None:
                # fallback to zeros if not available
                gunit_n = np.zeros((layer.units,), dtype=np.float32)
            else:
                # The code above stored raw grad unit importance; we need normalized version
                # reuse the same norm() helper from compute_utilities or reimplement:
                arr = np.asarray(gunit_n, dtype=np.float32)
            ...
            # ablation (delta_loss): higher delta => more important
            ...
            beta = float(self.config.fisher_weight)
            gamma = float(self.config.ablation_weight)
            unit_score = alpha * gunit_n + beta * f + gamma * a_n

            # store raw unit_score on layer (for debugging / downstream)
            ...
            # final utility exposed to pruning logic should be EMA-smoothed score
