"""
adaptive_overparam_experiment_v14.py
...
class EvoCompressor:
    ...
    def evaluate_and_compress(self, train_loss=None, val_loss=None):
        ...
        # capture activations for masked layers (keys are like f"{layer.name}_{hex(id(layer))}")
        ...
        # utilities per masked-layer instance key (same keys as entropies)
        utilities = {}
        ...
        # iterate over utilities (these are only layers with non-empty activations)
        for unique_name, score in utilities.items():
            # find the layer object that matches the unique name
            ...
            # --- update connection age for currently alive connections ---
            conn_age = layer.conn_age.numpy()
            # increment only positions where conn_mask==1
            ...
            if len(alive_idx) > 0:
                ...
                if n_regrow > 0:
                    ...
                    # actually perform regrowth
                    layer.regrow_units(regrow_idx)
                    print(f"[DEBUG] {layer.name}: pruned {len(prune_idx)} -> regrew {len(regrow_idx)} (stochastic)")
    
            # ---- connection-level guided rewiring ----
            W = layer.kernel.numpy()  # snapshot before rewiring (for logging)
            conn_mask = layer.conn_mask.numpy()
            ...
            if n_rewire > 0:
                ...
                # prune weakest alive connections by abs magnitude
                # guard k
                k_prune = min(n_rewire, np.count_nonzero(flat_absW))
                if k_prune > 0:
                    low_idx = np.argpartition(flat_absW, k_prune)[:k_prune]
                ...
                # compute per-output (column) utility from utilities dict
                ...
                # input-side proxy: mean abs incoming weight (normalized)
                ...
                if dead_pairs.size == 0:
                    ...
                    # desirability: prefer out_u (50%) and in_u (50%) — adjust alpha/beta as you like
                    alpha_out = 0.6
                    beta_in = 0.4
                    di = dead_pairs[:, 0]
                    dj = dead_pairs[:, 1]
                    ...
                    if k > 0:
                        ...
                        # perform regrowth and reset ages for regrown connections
                        layer.regrow_connections(regrow_pairs)
                        # reset ages to 0 for regrown positions
                        conn_age = layer.conn_age.numpy()
            ...
            df_rows.append({
                ...
                'utility_mean': float(utilities[unique_name].mean()) if unique_name in utilities else 0.0,
                'avg_conn_mag': avg_conn_mag
            })
    
        # evaluate model and finalize
