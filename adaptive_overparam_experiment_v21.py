"""adaptive_overparam_experiment_v21.py"""
...
# ---------------------------------------------------------------------------
# Configuration dataclass (extended for layer-wise control)
# ---------------------------------------------------------------------------
@dataclass
class EvoConfig:
...
class EvoCompressor:
    ...
    def _apply_prune_layerwise(self, stored_prune_info, baseline_weights, rel_tol):
        ...
        - Iterate layers in order of potential compression (descending n_prune) to get biggest wins early
        - For each layer: set baseline weights, apply only that layer's prune, evaluate
        - If the layer's prune keeps val_loss within rel_tol relative to *current* baseline, accept and update baseline
        - Record per-layer val_loss and rel_delta for use in selective regrowth
        """
        self.log.info('[LAYERWISE] Starting sequential layer-wise prune evaluation')
        accepted = []
        rejected = []
        ...
        for unique_name, info in items:
            ...
            # apply prune for only this layer
            self._apply_prune_scenario({unique_name: info}, 'both')

            metrics = self._evaluate_val()
            v = float(metrics[0])
            delta = (v - baseline_val) / (abs(baseline_val) + 1e-12)

            info['val_loss_if_pruned'] = v
            info['rel_delta'] = delta

            if v <= baseline_val * (1.0 + rel_tol):
                ...
                baseline_val = v
                self.log.info(f'[LAYERWISE] ACCEPTED {unique_name}: rel_delta={delta:.6f} -> new baseline_val={baseline_val:.9f}')
            else:
                # reject this layer's prune; revert
                rejected.append(unique_name)
                self.model.set_weights(current_baseline)
                self.log.info(f'[LAYERWISE] REJECTED {unique_name}: rel_delta={delta:.6f}')
    ...
    def _search_or_apply_scenarios(self, stored_prune_info, total_alive_pre):
        ...
        # fallback to old behavior (network-level scenarios)
